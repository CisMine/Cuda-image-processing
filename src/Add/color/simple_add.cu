...
__global__ void simple_add(const unsigned char *bgr1, const unsigned char *bgr2, unsigned char *result, int width, int height)
{
    ...
    if (x < width && y < height)
    {
        ...
        result[idx] = (sumB > 255) ? 255 : static_cast<unsigned char>(sumB);
        result[idx + 1] = (sumG > 255) ? 255 : static_cast<unsigned char>(sumG);
        result[idx + 2] = (sumR > 255) ? 255 : static_cast<unsigned char>(sumR);
    }
}
