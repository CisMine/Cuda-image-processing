...
__global__ void simple_add(const unsigned char *img1, const unsigned char *img2, unsigned char *result, int width, int height)
{
    ...
    if (x < width && y < height)
    {
        ...
        result[idx] = (sum > 255) ? 255 : static_cast<unsigned char>(sum);
    }
}
