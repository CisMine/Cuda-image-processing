...
__global__ void simple_sobelX(const unsigned char *srcImage, unsigned char *result, int width, int height)
{
...
int main()
{
    ...
    simple_sobelX<<<blocks, threads>>>(d_image, d_result, width, height);
